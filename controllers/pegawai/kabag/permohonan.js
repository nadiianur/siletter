...
const deleteDisposisi = async (req, res) => {
    try {
        ...
        if (!pegawaiRole1) {
            return res.status(404).json({ success: false, message: 'Pegawai dengan id_role 1 tidak ditemukan' });
        }
        ...
        const { id_surat_masuk } = disposisi;
        ...
        if (disposisiTerhubung) {
            return res.status(400).json({ success: false, message: 'Disposisi tidak dapat dihapus, telah disetujui Kepala Biro' });
        }
